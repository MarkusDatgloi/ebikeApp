...
class ebikeBleDelegate extends Ble.BleDelegate
{
    ...
    //==============================================
    // All functions called from this class and other classes
	...
	function onSettingsChanged(){
		...
		if (ebike.lastLock && ebike.lastMACArray != null && connectedMACArray != null &&
		   !sameMACArray(ebike.lastMACArray, connectedMACArray)) {
			bleDisconnect();
		}
    ...
    function onScanResults(scanResults) {
    	...
    	if (profilesRegistered < 3 ) {
    	    scanCount++;
    	    // Give onProfileRegister some scans before it is an error.
    	    if ( scanCount > 5 && ebike.errorReport == null){
    	    	ebike.errorReport = "ErrPrf:" + (profilesRegistered - 3);
    	    }
    	} 
		if (!wantScanning) {
